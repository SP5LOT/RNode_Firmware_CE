  ...
  if (modems_installed && !console_active) {
    if (!radio->getRadioOnline()) {
        if (!radio->getRadioLock() && hw_ready) {
          // Send radio=ON response early so rnsd's 250ms validation window
          // receives the state before begin() completes (~270ms calibration)
          kiss_indicate_radiostate_value(radio, 0x01);
          if (!radio->begin()) {
            // The radio could not be started.
            // Undo optimistic state and indicate failure
  ...
  if (IN_FRAME && sbyte == FEND && 
    ...
    if (frame_len == 0 && command == CMD_UNKNOWN) {
      ...
      if (sbyte == 0xFF) {
        ...
        // Pre-echo all stored radio parameters immediately before SPI operations
        // so rnsd's 250ms validateRadioState() window receives them without delay
        kiss_indicate_frequency(selected_radio);
        kiss_indicate_bandwidth(selected_radio);
        kiss_indicate_txpower(selected_radio);
        kiss_indicate_spreadingfactor(selected_radio);
